...
def sync_tags():
    ...
    tags = json.loads(get("https://yande.re/tag.json?limit=0").text)  # getting json file with all tags
...
def get_tags(search_query):
    ...
    page = json.loads(get(f"https://yande.re/tag.json?name={search_query}&limit={max_count}&order=count").text)
    for tag in page:
        if tag["count"] > 0:
            print(_get_tag(tag))
...
def get_images(search_tag, page_num):
    ...
    json_page = json.loads(get(f"https://yande.re/post.json?limit={limit}&page={page_num}&tags={search_tag}").text)
